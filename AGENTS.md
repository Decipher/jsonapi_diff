...
This is a JSON:API Diff template for creating contributed modules or themes. The project provides a complete development environment with CI configuration, testing setup, and deployment automation for publishing to Drupal.org.
...
- `jsonapi_diff.*` - Template extension files
- `JsonapiDiffService.php` - Main service class template
...
1. Run `php init.php` to customize template for JSON:API Diff
2. Build environment: `make build` or `ahoy build`
3. Develop JSON:API Diff code in `src/`
...
- SQLite database created in `/tmp/site_jsonapi_diff.sqlite`
